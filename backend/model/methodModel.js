...
class MethodModel {
    ...
    async getMethod(id) {
      ...
      method.additionalInfo = await this.methodsQueries.getMethodAdditionalInfo(
        id
      );
      return method;
    }
    ...
    async addMethod(method) {
      const resultMethod = await this.methodsQueries.addMethodMainInfo(
        method.title,
        method.description
      );
      for (let index = 0; index < method.additionalInfo.length; index++) {
        const element = method.additionalInfo[index];
        await this.methodsQueries.addMethodAdditionalInfo(
          Number(resultMethod.id),
